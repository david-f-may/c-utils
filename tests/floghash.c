/* Source File: floghash.c */

/*
 * [BeginDoc]
 *
 * \subsection{floghash.c}
 *
 * Source: floghash.c
 * Script: floghash.sh
 *
 * The floghash program demonstrates and tests creating and using an
 * nsort hash object.  The test script here creates a number of random strings
 * and those are then hashed and added to the object.  Then, they are all
 * searched for to insure that they are there (a warning is printed if any
 * are not found).  Then, bogus items are created that are known \emph{not}
 * to be there and these are searched in the hash.
 *
 * [EndDoc]
 */
#include <stdio.h>
...
int main (int argc, char *argv[])
{
  ...
  /*
   * [BeginDoc]
   * The following is how the hash object is declared in the C program:
   * [Verbatim] */
   ...
   * [BeginDoc]
   *
   * The hash object here is created using nsort_hash_create() as follows:
   * [Verbatim] */

  hsh = nsort_hash_create ();
  if (hsh == 0) {
    nsort_show_error(str, ERROR_LEN);
    printf ("\n\n***Error: nsort_hash_create(): %s\n", str);
    return _ERROR_;
  }
  status = nsort_hash_init (hsh, testCompare, 0);
  if (status == _ERROR_) {
  ...
  for (i = 0; cpp[i] && i < MAXDATA; i++) {
     ...
     * [BeginDoc]
     *
     * The following is how you add an item to the hash:
     * [Verbatim] */
    ...
    if (status == _ERROR_) {
      if (hsh->hashError == SORT_UNIQUE) {
        ...
        printf ("\n\n***Error: nsort_hash_add_item (hsh, %s): %s",
            cpp[i], sortErrorString[hsh->hashError]);
        nsort_hash_del (hsh);
        nsort_hash_destroy (hsh);
        free (cp);
        free (cpp);
    ...
    if (hsh->numCompares > num)
      num = hsh->numCompares;
    // To get a legitimate estimate of how quickly the hash allows adds,
    // I should move this to its own loop.  Not strictly necessary, though,
    // as lists are pretty quick.
    ...
    data = (char*)malloc (strlen (cpp[i])+1);
    if (0 == data) {
  ...
  while (lnk != lh->tail) {
     ...
     * [BeginDoc]
     *
     * Here's how we search through the hash object for items:
     * [Verbatim] */
    ...
    if (0 == data) {
      if (hsh->hashError == SORT_NOERROR)
        ...
        printf ("\n\n***Error: nsort_hash_find_item(hsh, %s): %s\n",
            (char*)lnk->data, sortErrorString[hsh->hashError]);
   ...
   * [BeginDoc]
   *
   * Finally, here's how we clean up:
   * [Verbatim] */
