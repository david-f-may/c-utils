 ...
 * Script: flognsrt.sh
 *
 * The flognsrt program is identical to flogsrt in terms of functionality with the
 * exception that duplicates are handled by flognsrt.  The nsort routine allows you 
 * to determine at the point of initialization whether the initialized sort object
 * will permit duplicates.
 *
 * [EndDoc]
 */
#include "sorthdr.h"
...
int main (int argc, char *argv[])
{
   ...
   * [BeginDoc]
   *
   * The following is the creation and initialization of the sort object.
   * [Verbatim] */
   ...
   * [BeginDoc]
   * 
   * Notice the familiar routine...we are going to create the object and then
   * initialize it.  The difference between this initialization and the one in
   * flogsrt is the third parameter to nsort_init().  Here it is FALSE; in
   * flogsrt it was TRUE.  This parameter is called isUnique in the function
   * definition and if it is TRUE, unique constraints are mandated; in other words,
   * duplicates are not allowed if this parameter is TRUE.  Here, however, it is
   * FALSE, which means that duplicates are allowed.
   *
   * The shell script flognsrt.sh, which drives this program, uses a program to
   * specifically make \emph{lots} of duplicates.  In fact, the script creates an
   * initial file set that consists of 25,000 items which are almost always unique.
   * Then, it uses a program called mkdups.c which creates duplicates.  You can look
   * at flognsrt.sh and mkdups to see how this is accomplished.  The end result is
   * that duplicate handling is fairly exhaustively checked.
   *
   * [EndDoc]
   */
