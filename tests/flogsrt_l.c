 ...
 * [BeginDoc]
 *
 * \subsection{flogsrt_l.c}
 *
 * Program: flogsrt_l.c
 * Shell: flogsrt_l.sh
 *
 * The flogsrt_l program is the same as the flogsrt program except that the
 * sort object is allocated on the stack rather than on the heap.  I will point
 * out the differences from the flogsrt program.
 *
 * [EndDoc]
 */
...
int main (int argc, char *argv[])
{
  ...
  /*
   * [BeginDoc]
   * Notice that the variable is declared as an nsort_t object and not
   * as a pointer to an nsort_t object:
   * \index{nsort_t}
   * [Verbatim] */
   ...
   * [BeginDoc]
   *
   * We don't need to call nsort_create() because the object is created on the
   * stack and when we call nsort_init(), we pass the address of the nsort_t
   * object.  This will be true throughout the remainder of the code.  Also,
   * when we reference a member of the nsort_t structure, we will use `.'
  ...
  while (cpp[counter] != 0 && counter < totalcount) {
     ...
     * [BeginDoc]
     * 
     * All this effort to handle errors seems cumbersome and is most often ``useless''.
     * However, it is not useless.  It is critical.  One of the principles of good
     * coding is that you need to check the diagnostics of \emph{any and all}
     * functions that provide diagnostics.  If a function can return an error,
  ...
  printf ("Removing %d items from the sort (each '.' is 100 items)\n", counter);
  {
    ...
    for (i = 0; i < counter; i++) {
      ...
      if (found == 0) {
        ...
        printf ("\n\n***Error: nsort_remove_item (srt, srt.lh->tail->prev->prev): %s\n",
            str);
        return _ERROR_;
      }
